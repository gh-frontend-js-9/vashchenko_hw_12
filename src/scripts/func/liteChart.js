 ...
 * @link https://codyshop.ru/litechart-library-for-chart-creation/
 */
export function liteChart(id, settings) {
...
liteChart.prototype.draw = function(restartAnimation) {
  ...
  let area = function(chart,id, chords, stroke, width, fill) {
    ...
    var path = getPath(chart,chords);

    if (id.indexOf("area") !== -1) {
      var first = chords[Object.keys(chords)[0]];
  ...
  let blurEffect = function(chart,id, stroke, chords,chordsAnimation) {
    ...
    let b = createElement( "path", {"id":id,"d":path,"fill":"transparent","stroke":stroke,"opacity":"0.7","stroke-width":chart.line.width,"filter":"url(#" + id + ")"} );
    ...
    if(chart.animate.show) {
      var animation = startAnimaton(chart,b,chordsAnimation);
  ...
  let startAnimaton = function(chart,parentElement,chords) {
    // Flat line on axis X
    var lineD0 = getPath(chart,chords);

    // Current line
    var lineD1 = parentElement.getAttribute("d");

    parentElement.setAttribute("d", lineD0);
    var animation = animate("d", lineD0, lineD1, chart.animate.duration, "freeze");
    return animation;
  }
  ...
  let drawGridX = function(chart) {
    ...
    while(currentLineValue <= axisYMaxValue) {
      ...
      var val = addComma(currentLineValue);
      ...
      if(chart.gridX.label.show) {
        if(currentLineHeight >= fontSize / 2) {
          var id = chart.id + "-grid-x-" + gridIndex + "-label"
    ...
    chart.legends.list.forEach(function(legend) {
      ...
      if ( "gradient" === chart.fill || "origin" === chart.fill ) {
        ...
        if ( "gradient" === chart.fill ) {
          var linear = linearGradient(legend.stroke, legend.stroke);
          g.appendChild(linear);
        }

       var fill = "origin" === chart.fill ? legend.stroke : "url(#"+legend.stroke+")";
        var grad = area(chart,"area-" + legendsIndex
          , chords
          , legend.stroke
          , chart.line.width
          , fill);
        ...
        if(chart.animate.show) {
          var grads = area(chart,"area-" + legendsIndex
            , chords
            , legend.stroke
            , chart.line.width
            , fill);
      ...
      legend.values.forEach(function(value) {
        ...
        if(labelCount > valuesIndex) {
          ...
          if(chart.point.show
            ...
            var stroke = chart.point.stroke === null ? legend.stroke : chart.point.stroke;
            ...
            if(chart.animate.show) {
              ...
              var pointMove = animate("cy", getAxisXPosition(chart), y, chart.animate.duration, "freeze");
