...
function pieChart(chartOut, percentOut, valueOut, paramName, valuePercent, value, renderSpeed, size, colorBackground, colorChart) {
  if (!size) {
    size = 50;
  }
  if (!colorBackground) {
...
// Line chart Data
function lineChart (label, agrigationData) {
  // Set labels ["Monday", "Thu", "Wed", "Tue", "Fri", "Sat", "Sun"]
  paramLineChart.setLabels(label);

  // Set legends and values [100, 200, 450, 400, 500, 300, 700]
  paramLineChart.addLegend({"name": "Day", "stroke": "#2196f3", "values": agrigationData});
